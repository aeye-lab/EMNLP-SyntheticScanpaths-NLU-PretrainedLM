...
from utils import load_corpus, ET_Sentiment2_Textonly_Dataset, count_parameters
...
def main():
	...
	for train_idx, test_idx in kf.split(split_list):
		...
		for epoch in range(starting_epoch, args.num_train_epochs):
			...
			if np.mean(val_loss) < old_score:
				...
				torch.save(model.state_dict(), '{}/CELoss_ETSA_finetune_BERTbase_{}_fold{}.pth'.format(args.output_dir, args.max_train_samples, fold_indx))
		...
		#evaluation
		model.eval()
		model.load_state_dict(torch.load(os.path.join(args.output_dir,f'CELoss_ETSA_finetune_BERTbase_{args.max_train_samples}_fold{fold_indx}.pth'), map_location='cpu'))
		model.to(args.gpu)
		ref = []
		prob = []
		for step, batch in enumerate(test_dataloaderr):
			with torch.no_grad():
				...
				prob.extend(outputs.logits.softmax(dim=-1).detach().cpu().numpy()[:,1].tolist())

		fpr, tpr, thresholds = metrics.roc_curve(ref, prob)
	...
	#save results
	with open(f'{args.output_dir}/res_BERTbase_ETSA_{args.max_train_samples}.pickle', 'wb') as handle:
