...
class ETSA_PLM_AS(nn.Module):
    def __init__(self, init_model):
        super(ETSA_PLM_AS, self).__init__()
        self.model = init_model
        #freeze the classification layer parameters in Bert model
        #instead we can use BertModel(config, add_pooling_layer=False)
    ...
    def forward(self, batch):
        ...
        x_sp = torch.gather(x, 1, gaze_pos.unsqueeze(2).repeat(1,1,768).long())
...
class Joint_Gaze_LM(nn.Module):
	def __init__(self, sp_gen_model, gaze_LM):
		super(Joint_Gaze_LM, self).__init__()
		self.sp_gen_model = sp_gen_model
		self.gaze_LM = gaze_LM

	def convert_word_pos_seq_to_token_pos_seq(self, word_pos, sn_len, word_ids_sn):
		#Find the number "sn_len+1" -> the end point
		stop_mask = (word_pos == (sn_len+1).unsqueeze(1))
		stop_mask = ~(stop_mask.cumsum(dim=1).cumsum(dim=1) == 1).cumsum(dim=1).bool()
		...
		for b in range(word_pos.shape[0]):
			...
			valid_pos_seq = torch.masked_select(word_pos[b,:], stop_mask[b,:])
			#valid_pos_seq = torch.cat((valid_pos_seq, sn_len[b].reshape(1)+1)) #add SEP token back
			...
			cur_token_pos = torch.tensor([0], dtype=torch.float64).to(word_pos.device) # fake CLS token for tensor concatenation
			for p in valid_pos_seq:
				...
				#cur_token_pos = torch.cat((cur_token_pos, torch.where(word_ids_sn[b]==p)[0]))#non-differentiable
	...
	def forward(self, batch, le):
		gaze_pos, sn_len = self.sp_gen_model(sn_emd = batch['input_ids'],
												sn_mask = batch['attention_mask'],
												word_ids_sn = batch['word_ids'],
												sn_word_len = batch['word_len'],
												le = le)

		gaze_token_pos = self.convert_word_pos_seq_to_token_pos_seq(word_pos=gaze_pos, sn_len=sn_len, word_ids_sn=batch['word_ids'])
...
class PLM_AS(nn.Module):
	def __init__(self, args, seed):
		...
		config = AutoConfig.from_pretrained(args.model_name_or_path, num_labels=args.num_labels, finetuning_task='sst2')
	...
	def forward(self, batch, gaze_pos):
		...
		#retrieve features according to scanpath ordering
		#x_sp = torch.gather(x, 1, gaze_pos.unsqueeze(2).repeat(1,1,768))#non-differentiable
...
class Eyettention_pretrain(nn.Module):
    def __init__(self, cf):
        ...
        #BERT encoder
        bert_encoder_config = BertConfig.from_pretrained(self.cf["model_pretrained"])
        bert_encoder_config.output_hidden_states=True
         # initiate Bert with pre-trained weights
        print("keeping Bert with pre-trained weights")
        self.bert = BertModel.from_pretrained(self.cf["model_pretrained"], config = bert_encoder_config)
        self.bert.eval()
        #freeze the parameters in Bert model
        for param in self.bert.parameters():
            param.requires_grad = False
